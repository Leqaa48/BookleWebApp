...
(function ($) {
    ...
    document.querySelectorAll('.quantity-input').forEach(input => {
        input.addEventListener('change', function () {
            ...
            fetch(`/Cart/UpdateQuantity?bookId=${bookId}&quantity=${quantity}`, {
                method: 'POST',
                headers: {
    ...
    $(document).ready(function () {
        // Fetch the cart item count when the page is loaded
        function updateCartItemCount() {
            $.get("/Carts/GetCartItemCount", function (data) {
                $("#cart-item-count").text(data); // Update the cart item count
            });
        }
    ...
    $(document).ready(function () {
        ...
        $(document).on("click", ".decrease-quantity", function () {
            ...
            const inputField = $(`#cart-item-quantity-${cartItemId}`);
            const currentQuantity = parseInt(inputField.val());

            if (currentQuantity > 1) {
                $.ajax({
                    type: "POST",
                    url: "/Carts/DecreaseQuantity",
                    data: { id: cartItemId },
                    success: function () {
        ...
        $(document).on("click", ".increase-quantity", function () {
            ...
            if (currentQuantity < 10) { // Example: Limit to max of 10
                $.ajax({
                    type: "POST",
                    url: "/Carts/IncreaseQuantity",
                    data: { id: cartItemId },
                    success: function () {
                        updateCart(cartItemId, currentQuantity + 1);
                    },
        ...
        function updateCart(cartItemId, newQuantity) {
            ...
            // Update totals
            $.get("/Carts/GetCartData", function (data) {
                $(".sub-price-total").text(`$${data.totalPrice}`);
    ...
    $(document).on('click', '.qtyminus', function () {
        ...
        if (currentQuantity > 1) {
            ...
            $('#quantity').val(quantityInput.val()); // Update hidden input
        }
    });
    ...
    $(document).on('click', '.qtyplus', function () {
        ...
        if (currentQuantity < 10) { // Limit to max of 10
            quantityInput.val(currentQuantity + 1);
            $('#quantity').val(quantityInput.val()); // Update hidden input
        }
    });

    // Update the hidden input with current quantity value whenever it's changed manually
